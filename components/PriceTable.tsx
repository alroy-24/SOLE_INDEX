import type { Offer, Sneaker } from "@/lib/types";
...
export function PriceTable({ sneaker }: { sneaker: Sneaker }) {
  ...
  const lowest = lowestOffer(sneaker);
...
function OfferRow({
  ...
  return (
    <div
      className={`grid grid-cols-[2.5rem_1fr] gap-x-4 gap-y-3 border-b border-r rule px-4 py-4 md:grid-cols-[2.5rem_1fr_auto_auto] md:items-center ${bg}`}
    >
      {/* rank */}
      <div className="numerals self-start pt-1 text-sm text-ash md:self-center md:pt-0">
        {offer.linkOnly ? "↗" : offer.inStock ? String(rank).padStart(2, "0") : "—"}
      </div>
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-xs text-ash">
          {offer.linkOnly
            ? `${r.note} · live price not indexed yet`
            : `${r.note} · ${r.domain} · updated ${timeAgo(offer.fetchedAt)}`}
        </p>
        {!offer.linkOnly && offer.inStock && offer.sizes && offer.sizes.length > 0 && (
      ...
      <div className="md:text-right">
        {offer.linkOnly || typeof offer.price !== "number" ? (
          <span className="text-sm text-ash">See on site</span>
        ) : (
          <>
            <div className="flex items-baseline gap-2 md:justify-end">
              ...
              {off && <span className="numerals text-xs text-drop">−{off}%</span>}
            </div>
            {offer.mrp && (
      ...
      <div className="md:pl-4 md:text-right">
        {offer.linkOnly ? (
          <a
            ...
            className="inline-flex w-full items-center justify-center gap-1.5 border border-line px-4 py-2.5 text-sm font-semibold uppercase tracking-wide text-ink-soft transition-colors hover:border-ink hover:text-ink md:w-auto"
          >
            Check price ↗
          </a>
        ) : offer.inStock ? (
