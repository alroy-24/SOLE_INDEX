...
const MOCK: Sneaker[] = RAW.map(build);

// Run every outbound link through the affiliate wrapper (no-op without env).
function withAffiliateLinks(list: Sneaker[]): Sneaker[] {
  return list.map((s) => ({
    ...s,
    offers: s.offers.map((o) => ({ ...o, url: affiliateUrl(o.retailerId, o.url) })),
  }));
}

// Prefer the real catalogue generated by `scripts/fetch-data.mjs`; fall back to
// the hand-authored mock if it hasn't been generated yet.
const REAL = catalogJson as unknown as Sneaker[];
const SNEAKERS: Sneaker[] = withAffiliateLinks(REAL.length > 0 ? REAL : MOCK);
